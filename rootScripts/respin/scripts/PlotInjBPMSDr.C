...
void PlotInjBPMSDr(Int_t runnum, vector<TString> vBPM, Int_t IHWPstatus, TString ucut){
  ...
  vector< TString > vInjBPM = vBPM;
  Int_t nBPM = vInjBPM.size();
  ...
  for(Int_t dd=0;dd<ndeti;dd++)
    {
      mul_tree->Draw(Form("1e6*asym_%sWS>>hista%d",vInjBPM[dd].Data(),dd),ucut,"goff");
      ...
      zero[dd]=0;
      // delete thishist;
      // mul_tree->Draw(Form("diff_%selli/1e3/2>>histe%d",vInjBPM[dd].Data(),dd),ucut,"goff");
      // objnam = Form("histe%d",dd);
      // thishist = (TH1D*)gDirectory->FindObject(objnam.Data());
      // Aelli[dd] = thishist->GetMean();
      // eAelli[dd]= thishist->GetMeanError();
      // rmsAelli[dd]= 1000*thishist->GetRMS();
      // delete thishist;
      mul_tree->Draw(Form("1e3*diff_%sY>>histx%d",vInjBPM[dd].Data(),dd),ucut,"goff");
      objnam = Form("histx%d",dd);
      thishist = (TH1D*)gDirectory->FindObject(objnam);
      Dx[dd] = thishist->GetMean();
      eDx[dd]= thishist->GetMeanError();
      // delete thishist;
      mul_tree->Draw(Form("1e3*diff_%sX>>histy%d",vInjBPM[dd].Data(),dd),ucut,"goff");
      ...
      //      cout<<vInjBPM[dd].Data()<<" Aq:"<<Aq[dd]<<"("<<rmsAq[dd]<<")+-"<<eAq[dd]<<"ppm Dx:"<<Dx[dd]<<"("<<rmsDx[dd]<<")+-"<<eDx[dd]<<"um Dy:"<<Dy[dd]<<"("<<rmsDy[dd]<<")+-"<<eDy[dd]<<"um "<<endl;
      // cout<<vInjBPM[dd].Data()<<" Aq[ppm](RMS)+-error,Dx[nm](RMS)+-err,Dy[nm](RMS)+-err,Aelli(RMS*10^3)+-error,Dr[nm]+-err,theta[deg]: "<<Aq[dd]<<" "<<rmsAq[dd]<<" "<<eAq[dd]<<" "<<1e3*Dx[dd]<<" "<<1e3*rmsDx[dd]<<" "<<1e3*eDx[dd]<<" "<<1e3*Dy[dd]<<" "<<1e3*rmsDy[dd]<<" "<<1e3*eDy[dd]<<" "<<Aelli[dd]<<" "<<rmsAelli[dd]<<" "<<eAelli[dd]<<" "<<1e3*Dr[dd]<<" "<<1e3*eDr[dd]<<" "<<theta[dd]<<endl;
    }
  ...
  for(Int_t k=0;k<numchains;k++){
    ...
    Drgraph->GetHistogram()->GetXaxis()->SetBinLabel(k+1,vInjBPM[k]);
  }
  Drgraph->SetMarkerStyle(21);
  Drgraph->SetMarkerColor(2);
  Drgraph->SetLineColor(2);
  Drgraph->Draw("AP");
  Drgraph->SetTitle(Form("IHWP=%d, Run %d, %s ",IHWPstatus, runnum, ucut.Data()));
  ...
  for(Int_t k=0;k<numchains;k++){
    thetagraph->GetHistogram()->GetXaxis()->SetBinLabel(k+1,vInjBPM[k]);
  }
  thetagraph->SetMarkerStyle(21);
  thetagraph->SetMarkerColor(2);
  thetagraph->SetLineColor(2);
  thetagraph->Draw("AP");
  thetagraph->SetTitle(Form("IHWP=%d, Run %d, %s ",IHWPstatus, runnum,ucut.Data()));
  ...
  for(Int_t k=0;k<numchains;k++){
    //      l2timevschain->GetHistogram()->GetXaxis()->SetBinLabel(k+1,names[k].c_str());
    Aqgraph->GetHistogram()->GetXaxis()->SetBinLabel(k+1,vInjBPM[k]);
  }
  ...
  Aqgraph->Draw("AP");
  Aqgraph->SetTitle(Form("IHWP=%d, Runs %d, %s ",IHWPstatus, runnum,ucut.Data()));
  ...
  for(Int_t k=0;k<numchains;k++){
    Aqrmsgraph->GetHistogram()->GetXaxis()->SetBinLabel(k+1,vInjBPM[k].Data());
  }
  ...
  Aqrmsgraph->Draw("AP");
  Aqrmsgraph->SetTitle(Form("IHWP=%d, Run %d, %s ",IHWPstatus, runnum, ucut.Data()));
  ...
  // for(Int_t k=0;k<numchains;k++){
  //   //      l2timevschain->GetHistogram()->GetXaxis()->SetBinLabel(k+1,names[k].c_str());
  //   Aelligraph->GetHistogram()->GetXaxis()->SetBinLabel(2*k+1,vInjBPM[k].Data());
  // }
  ...
  // Aelligraph->Draw("AP");
  // Aelligraph->SetTitle(Form("IHWP=%d, Runs %d, %s ",IHWPstatus, runnum,ucut.Data()));
  ...
  // for(Int_t k=0;k<numchains;k++){
  //   Aellirmsgraph->GetHistogram()->GetXaxis()->SetBinLabel(2*k+1,vInjBPM[k].Data());
  // }
  ...
  // Aellirmsgraph->SetTitle(Form("IHWP=%d, Run %d, %s ",IHWPstatus, runnum, ucut.Data()));
