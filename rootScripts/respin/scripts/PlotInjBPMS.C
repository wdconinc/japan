...
void PlotInjBPMS(Int_t runnum,vector<TString> vBPM , Int_t IHWPstatus, TString ucut){
  ...
  for(Int_t dd=0;dd<ndeti;dd++) {
    ...
    zero[dd]=0;
    // delete thishist;
    mul_tree->Draw(Form("1e3*diff_%sX>>histx%d",vInjBPM[dd].Data(),dd),ucut,"goff");
    ...
    // delete thishist;
    mul_tree->Draw(Form("1e3*diff_%sY>>histy%d",vInjBPM[dd].Data(),dd),ucut,"goff");
    ...
    // delete thishist;
    cout<<vInjBPM[dd]<<" Aq[ppm](RMS)+-error,Dx[nm](RMS)+-err,Dy[nm](RMS)+-err: "<<Aq[dd]<<" "<<rmsAq[dd]<<" "<<eAq[dd]<<" "<<1e3*Dx[dd]<<" "<<1e3*rmsDx[dd]<<" "<<1e3*eDx[dd]<<" "<<1e3*Dy[dd]<<" "<<1e3*rmsDy[dd]<<" "<<1e3*eDy[dd]<<endl;
  ...
  xygraph->SetTitle(Form("IHWP=%d, Run %d, %s ",IHWPstatus, runnum,ucut.Data()));
  ...
  for(Int_t k=0;k<numchains;k++){
    xygraph->GetHistogram()->GetXaxis()->SetBinLabel(k+1,vInjBPM[k]);
  }
  pad_buff->SetGrid();
  ...
  xyrmsgraph->Draw("AP");
  xyrmsgraph->SetTitle(Form("IHWP=%d, Run %d, %s ",IHWPstatus, runnum,ucut.Data()));
  xyrmsgraph->GetYaxis()->SetTitle("RMS um, blue=Dx, green=Dy");
  xyrmsgraph->GetHistogram()->GetXaxis()->Set(numchains,x1,x2);
  for(Int_t k=0;k<numchains;k++){
    xyrmsgraph->GetHistogram()->GetXaxis()->SetBinLabel(k+1,vInjBPM[k]);
  }
  pad_buff->SetGrid();
  ...
  for(Int_t k=0;k<numchains;k++){
    Aqrmsgraph->GetHistogram()->GetXaxis()->SetBinLabel(k+1,vInjBPM[k]);
  }
